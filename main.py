...
from tkinter import *
hunians = []
hunians.append(Apartemen("Saul Goodman", 3, 3, 100, 1000))
hunians.append(Rumah("Gustavo Fring", 5, 2, 400, 2250))
hunians.append(Indekos("Chuck McGill", "Howard Hamlin", 60, 500))
hunians.append(Rumah("Mike Ehrmantraut", 1, 4, 80, 1500))
root = Tk()
...
def details(index):
    ...
    d_luasTanah = Label(d_frame, text=" Luas Tanah (m2) : " + str(
        hunians[index].get_luasTanah()), anchor="w").grid(row=2, column=0, sticky="w")
    d_kapasitasListrik = Label(d_frame, text="Kapasitas Listrik (VA) : " + str(
        hunians[index].get_kapasitasListrik()), anchor="w").grid(row=3, column=0, sticky="w")
